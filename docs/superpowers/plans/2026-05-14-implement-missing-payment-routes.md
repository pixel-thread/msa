...
export async function getAllTransactions(associationId: string, filters: any) {
  ...
  const [transactions, total] = await Promise.all([
    prisma.paymentTransaction.findMany({
      where,
      include: { user: { select: { name: true, email: true, membershipNumber: true } } },
...
export async function getFinancialStats(associationId: string) {
  ...
  const [monthTotal, dues] = await Promise.all([
    prisma.paymentTransaction.aggregate({
      where: { associationId, status: "COMPLETED", paidAt: { gte: startOfMonth } },
      _sum: { amount: true },
    }),
...
export const GET = withAssociation(
  ...
  async (association, { query }, request) => {
    ...
    return SuccessResponse({ data: result.transactions, meta: result.pagination });
  },
);
```
...
export const GET = withAssociation(
  async (association, _, request, { params }) => {
    ...
    const isFinance = user.role.some(r => [UserRole.FINANCE, UserRole.SECRETARY, UserRole.PRESIDENT, UserRole.SUPER_ADMIN].includes(r));
...
export const GET = withAssociation(
  async (association, _, request, { params }) => {
    ...
    const receiptData = {
      ...
      memberInfo: { name: transaction.user.name, membershipNumber: transaction.user.membershipNumber },
