...
export const suspendMember = [
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      logger.info({ traceId, associationId: association.id }, 'POST /api/members/[memberId]/suspend - Request started');

      if (!user.role.includes(UserRole.PRESIDENT)) throw new ForbiddenError('Insufficient permissions');

      logger.info({ traceId, userId: user.id }, 'POST /api/members/[memberId]/suspend - User authorized');
