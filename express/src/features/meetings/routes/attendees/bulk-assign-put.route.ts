...
export const putBulkAssignAttendees = [
  validate({ params: MeetingParamsSchema, body: BulkAssignAttendeesSchema }),
  async (req: Request, res: Response, _next: NextFunction) => {
    const traceId = (req.headers['x-trace-id'] as string) || '';
    const association = await getAssociation(req);
    const meetingId = req.params.meetingId as string;
    logger.info({ traceId, meetingId, associationId: association.id }, 'PUT /api/meetings/[meetingId]/attendees - Request started');
    ...
    logger.info({ traceId, userId: user.id, role: user.role, meetingId }, 'PUT /api/meetings/[meetingId]/attendees - User authorized');
    logger.info({ traceId, meetingId }, 'PUT /api/meetings/[meetingId]/attendees - Bulk assigning attendees');
    ...
    logger.info({ traceId, meetingId, assigned: result.assigned.length, skipped: result.skipped.length }, 'PUT /api/meetings/[meetingId]/attendees - Success');
    return success(res, {
      data: result,
