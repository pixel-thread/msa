...
import { CreateAssociationSchema, UpdateAssociationSchema, AddAssociationMemberSchema } from '@src/features/associations/validators';
...
export const getAssociationByUser = [
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      logger.info({ traceId, userId: user.id, roles: user.role }, 'GET /api/associations - User authorized');
...
export const postAssociationCreate = [
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      logger.info({ traceId, userId: user.id, roles: user.role }, 'POST /api/associations - User authorized');
      const existing = await findFirstAssociation({
        where: { OR: [{ slug: req.body?.slug, status: 'ACTIVE' }, { name: req.body?.name, status: 'ACTIVE' }] },
        take: 1,
      });
      if (existing) {
        logger.error({ traceId, slug: req.body?.slug, name: req.body?.name }, 'POST /api/associations - Association Already Exists');
...
export const getCurrentAssociation = [
  async (req: Request, res: Response, next: NextFunction) => {
    const traceId = (req.headers['x-trace-id'] as string) || '';
    try {
      const association = await getAssociation(req);
      ...
      const user = await withRole(req, UserRole.MEMBER);
      logger.info({ traceId, userId: user.id, roles: user.role }, 'GET /api/associations/current - User authorized');
      const currentAssociation = await findUniqueAssociation({ where: { id: association.id } });
      logger.info({ traceId, associationId: association.id }, 'GET /api/associations/current - Success');
      return success(res, { data: currentAssociation });
    } catch (e) { next(e); }
  },
];

export const getAssociationDetail = [
  async (req: Request, res: Response, next: NextFunction) => {
    const traceId = (req.headers['x-trace-id'] as string) || '';
    try {
      logger.info({ traceId, associationId: req.params.associationId }, 'GET /api/associations/[associationId] - Request started');
      const user = await withRole(req, UserRole.SUPER_ADMIN);
      logger.info({ traceId, userId: user.id, roles: user.role }, 'GET /api/associations/[associationId] - User authorized');
      const association = await findUniqueAssociation({ where: { id: req.params.associationId } });
      if (!association) {
        logger.error({ traceId, associationId: req.params.associationId }, 'GET /api/associations/[associationId] - Association not found');
        throw new NotFoundError('Association not found');
      }
      logger.info({ traceId, associationId: req.params.associationId }, 'GET /api/associations/[associationId] - Success');
      return success(res, { data: association, message: 'Association found successfully' });
    } catch (e) { next(e); }
  },
];

export const patchAssociationDetail = [
  validate({ body: UpdateAssociationSchema }),
  async (req: Request, res: Response, next: NextFunction) => {
    const traceId = (req.headers['x-trace-id'] as string) || '';
    try {
      logger.info({ traceId, associationId: req.params.associationId }, 'PATCH /api/associations/[associationId] - Request started');
      const user = await withRole(req, UserRole.SUPER_ADMIN);
      logger.info({ traceId, userId: user.id, roles: user.role }, 'PATCH /api/associations/[associationId] - User authorized');
      const existing = await findUniqueAssociation({ where: { id: req.params.associationId } });
      if (!existing) {
        logger.error({ traceId, associationId: req.params.associationId }, 'PATCH /api/associations/[associationId] - Association Not Found');
        throw new NotFoundError('Association Not Found');
      }
      if (req.body?.slug !== existing.slug || req.body?.name !== existing.name) {
        const conflict = await findFirstAssociation({
          where: { id: { not: req.params.associationId }, OR: [{ slug: req.body?.slug }, { name: req.body?.name }] },
          take: 1,
        });
        if (conflict) {
          logger.error({ traceId, slug: req.body?.slug, name: req.body?.name }, 'PATCH /api/associations/[associationId] - Association conflict');
          throw new ConflictError('Association with this slug or name already exists');
        }
      }
      const updated = await updateAssociation({ where: { id: req.params.associationId }, data: req.body as CreateAssociationInput });
      logger.info({ traceId, associationId: req.params.associationId }, 'PATCH /api/associations/[associationId] - Success');
      return success(res, { data: updated, message: 'Association updated successfully' }, 200);
    } catch (e) { next(e); }
  },
];

export const postDeactivateAssociation = [
  async (req: Request, res: Response, next: NextFunction) => {
    const traceId = (req.headers['x-trace-id'] as string) || '';
    try {
      logger.info({ traceId, associationId: req.params.associationId }, 'POST /api/associations/[associationId]/deactivate - Request started');
      const user = await withRole(req, UserRole.SUPER_ADMIN);
      ...
      const updatedAssociation = await updateAssociation({ where: { id: associationId }, data: { isActive: false } });
      logger.info({ traceId, associationId }, 'POST /api/associations/[associationId]/deactivate - Success');
      return success(res, { data: updatedAssociation, message: 'Association deactivated successfully' });
    } catch (e) { next(e); }
  },
];

export const postUploadLogo = [
  async (req: Request, res: Response, next: NextFunction) => {
    const traceId = (req.headers['x-trace-id'] as string) || '';
    try {
      const association = await getAssociation(req);
      ...
      const uploadResult = await uploadToBucket(file, `associations/logos/${association.slug}`, traceId);
      await prisma.association.update({ where: { id: association.id }, data: { logo: uploadResult.url } });
      logger.info({ traceId, associationId: association.id }, 'POST /api/associations/[associationId]/logo - Success');
      return success(res, { data: { key: uploadResult.key, url: uploadResult.url }, message: 'Logo uploaded successfully' }, 201);
    } catch (e) { next(e); }
  },
];
...
export const postAddMember = [
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      const association = await getAssociation(req);
      logger.info({ traceId, targetMemberId: req.body?.memberId, associationId: req.params.associationId }, 'POST /api/associations/[associationId]/members - Request started');
      ...
      if (!existingMember) {
        logger.error({ traceId, targetMemberId: req.body.memberId }, 'POST /api/associations/[associationId]/members - Member not found');
        throw new NotFoundError('Member not found');
      }
      if (existingMember.associationId === req.params.associationId) {
        logger.error({ traceId, targetMemberId: req.body.memberId, associationId: req.params.associationId }, 'POST /api/associations/[associationId]/members - Member already in this association');
      ...
      logger.info({ traceId, targetMemberId: req.body.memberId, associationId: association.id }, 'POST /api/associations/[associationId]/members - Success');
