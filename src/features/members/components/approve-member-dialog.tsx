...
type ApproveMemberForm = z.infer<typeof ApproveMemberSchema>;
...
export function ApproveMemberDialog({
  ...
  const form = useForm<ApproveMemberForm>({
    resolver: zodResolver(ApproveMemberSchema),
    defaultValues: {
      memberTypeId: "",
      role: "MEMBER",
    },
  ...
  const onSubmit = (data: ApproveMemberForm) => {
    if (!member) return;
