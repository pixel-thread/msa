...
export function useMemberTypeColumns({
  onEdit,
  onDelete,
}: UseMemberTypeColumnsOptions) {
  const columns: ColumnDef<MemberType>[] = [
    {
      accessorKey: "level",
      header: "Level",
      cell: ({ row }) => (
        <span className="text-sm font-medium">Level {row.original.level}</span>
      ),
    },
    {
    ...
    },
    {
      ...
      header: "Plans",
      cell: ({ row }) => (
        <span className="text-sm">{row.original._count.subscriptionPlans}</span>
      ),
    },
    {
      id: "actions",
      header: "Actions",
      cell: ({ row }) => (
        <MemberTypeActionsCell
          memberType={row.original}
          onEdit={onEdit}
          onDelete={onDelete}
        />
      ),
