...
export function useConsentColumns({
  onViewDetail,
  onEdit,
  onDelete,
}: UseConsentColumnsOptions) {
  const columns: ColumnDef<ConsentRecord>[] = [
    ...
    },
    {
      ...
      header: "Purpose",
      cell: ({ row }) => (
        <span className="text-sm">{purposeLabels[row.original.purpose]}</span>
      ),
    },
    {
      accessorKey: "status",
    ...
    },
    {
      ...
      header: "Channel",
      cell: ({ row }) => (
        <span className="text-sm capitalize text-body">
