...
export const GET = withAssociation(
  {},
  async (_association, { traceId }, req) => {
    ...
    if (!userId) {
      logger.error(
        { traceId },
        "GET /api/auth/me - Unauthorized (missing x-user-id)",
      );
    ...
    if (env.NODE_ENV === "production") {
      ...
      if (cachedUser) {
        logger.info(
          { traceId, userId },
          "GET /api/auth/me - Success (cached user returned)",
        );
    ...
    if (!user || user.status !== "ACTIVE") {
      logger.error(
        { traceId, userId },
        "GET /api/auth/me - User not found or inactive",
      );
